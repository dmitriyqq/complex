...
export default class TestThreeProgram {
  constructor(model, mappings) {
    ...
    this.gridMaterial = new THREE.LineBasicMaterial({color: 0xaaaaaa, wireframe: true});
    this.xAxiesMaterial = new THREE.LineBasicMaterial({color: 0xff0000, linewidth: 2});
    this.yAxiesMaterial = new THREE.LineBasicMaterial({color: 0x00ff00, linewidth: 2});
    this.zAxiesMaterial = new THREE.LineBasicMaterial({color: 0x0000ff, linewidth: 2});
    ...
    const planeGrid = (mapLine) => {
      for (let i = 0; i < totalSize; i++) {
        for (let j = 0; j < totalSize; j++) {
          ...
          addLine(x+this.cellSize, y, x+this.cellSize, y+this.cellSize, mapLine, gridGeometry);
          addLine(x+this.cellSize, y+this.cellSize, x, y+this.cellSize, mapLine, gridGeometry);
